...
def _ensure_reply_to_column():
    ...
    if "reply_to_id" not in col_names:
        ...
        changed = True
    if "forwarded_from_chat_id" not in col_names:
        db.session.execute(text("ALTER TABLE messages ADD COLUMN forwarded_from_chat_id INTEGER"))
        changed = True
    if "forwarded_from_message_id" not in col_names:
        db.session.execute(text("ALTER TABLE messages ADD COLUMN forwarded_from_message_id INTEGER"))
        changed = True
    if "forwarded_from_title" not in col_names:
        db.session.execute(text("ALTER TABLE messages ADD COLUMN forwarded_from_title VARCHAR(200)"))
        changed = True
    if changed:
        db.session.commit()
