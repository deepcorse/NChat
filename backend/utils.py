...
def serialize_poll(poll, viewer_id=None):
    from .models import PollVote

    vote_counts = {o.id: len(o.votes) for o in poll.options}
    my_votes = []
    if viewer_id is not None:
        my_votes = [
            v.option_id for v in PollVote.query.filter_by(poll_id=poll.id, user_id=viewer_id).all()
        ]
    ...
    return {
        ...
        "correct_option_id": poll.correct_option_id if not poll.anonymous else None,
        "options": [
            {"id": o.id, "text": o.text, "votes": vote_counts.get(o.id, 0)}
...
def build_message_payload(message, viewer_id=None):
    reply_to = None
    ...
    return {
        ...
        "forwarded_from": {
            ...
            "title": message.forwarded_from_title,
        } if message.forwarded_from_message_id else None,
        "poll": serialize_poll(message.poll, viewer_id) if getattr(message, "poll", None) else None,
